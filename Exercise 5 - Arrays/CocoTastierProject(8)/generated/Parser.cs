...
out type);
	...
	void ProcDecl(string progName) {
		...
		Expect(15);
		Ident(out procName);
		obj = tab.NewObj(procName, proc, undef, undef, -1);
		if (procName == "main")
		  if (tab.curLevel == 0)
	...
	void ConstantDecl() {
		...
		Type(out type);
		Ident(out name);
		tab.NewObj(name, constant, type, undef, -1); 
		while (la.kind == 38) {
			Get();
			Ident(out name);
			tab.NewObj(name, constant, type, undef, -1); 
	...
	void VarDecl() {
		int type; string name; int subcategory = scalar; int maxIndex = -1; 
		Type(out type);
		if (la.kind == 24) {
			Get();
			subcategory = array; 
		...
		Ident(out name);
		tab.NewObj(name, var, type, subcategory, maxIndex); 
		Expect(27);
	}
...
out typeR);
	...
	static readonly bool[,] set = {
		...
		{x,x,T,x, x,x,x,x, x,x,x,x, x,x,x,x, T,x,x,x, x,x,x,x, x,x,x,x, T,x,T,T, T,T,x,x, x,x,x,x, x},
